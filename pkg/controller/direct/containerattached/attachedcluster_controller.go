...
func loggingUnaryInterceptor2() grpc.UnaryClientInterceptor {
	return func(
		ctx context.Context,
		method string,
		req, reply interface{},
		...
		log.Printf("Invoking method: %s, Request: %v", method, req)
		...
		if err != nil {
			...
			log.Printf("Response from %s: %v", method, reply)
		}
...
func (m *modelContainerAttachedCluster) client(ctx context.Context, endpoint string) (*gcp.AttachedClustersClient, error) {
	// Configure options with the HTTP client and endpoint
	opts := []option.ClientOption{
		//option.WithHTTPClient(httpClient),
		option.WithGRPCDialOption(grpc.WithUnaryInterceptor(loggingUnaryInterceptor2())),
